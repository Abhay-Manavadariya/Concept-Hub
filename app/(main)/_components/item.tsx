...
interface ItemProps {
  id?: Id<"documents">; //"?" means optional
  documentIcon?: string;
  active?: boolean;
  expanded?: boolean;
  isSearch?: boolean;
...
const Item = ({
  ...
  const onCreate = (event: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
    ...
    const createDocumentPromise = create({
      title: "Untitled",
      parentDocument: id,
    }).then((documentId) => {
      if (!expanded) {
  ...
  return (
    ...
    >
      ...
      {isSearch && (
        // The <kbd> element is used to identify text that represents user keyboard input
        <kbd
          ...
          <DropdownMenu>
            ...
            >
              ...
              <div className="line-clamp-1 text-sm text-muted-foreground p-2">
                Last edited by: {user ? user.fullName : null}
              </div>
            </DropdownMenuContent>
          </DropdownMenu>
          <div
